...
def run_basic_metrics_controller_get_all_metrics_by_sensor_id_and_timestamp_for_all_metric_types():
    ...
    assert len(session.query(FullnessMetric).filter_by(sensor_id=fullness_sensor.sensor_id).all())==len(get_fullness_by_sensor_id_and_timestamp(fullness_sensor.sensor_id, datetime(1,1,1), datetime.now()+timedelta(days=10)))
    assert len(session.query(UsageMetric).filter_by(sensor_id=usage_sensor.sensor_id).all())==len(get_usage_by_sensor_id_and_timestamp(usage_sensor.sensor_id, datetime(1,1,1), datetime.now()+timedelta(days=10)))
    assert len(session.query(WeightMetric).filter_by(sensor_id=weight_sensor.sensor_id).all())==len(get_weight_by_sensor_id_and_timestamp(weight_sensor.sensor_id, datetime(1,1,1), datetime.now()+timedelta(days=10)))
